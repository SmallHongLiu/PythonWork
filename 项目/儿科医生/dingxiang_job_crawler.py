...
class CrawJob(object):
    ...
    def get_job_id(self, wd):
        with open('area_code.txt', 'r') as f:
            ...
            self.area_code = eval(self.area_code)  # eval执行一个字符串表达式，并返回表达式的值
            ...
            while retry:
                try:
                    ...
                    res_job = requests.get(url=detail_url,
                                       ...
                                       proxies=proxies,
                                       timeout=3)
                    detail_data = res_job.json()
                    if detail_data['success']:
                        item = detail_data['results']['entwork']
                        item['area'] = area
