...
class AQI(object):
    ...
    def crawl_aqi(self, sleep_time=3600):
        ...
        while 1:
            ...
            timestamp = parsed_text.xpath('/html/body/div[3]/p/i/text()')[0]\
                .replace('年', '-').replace('月', '-').replace('日', '-').replace('时', ':00:00')
            ...
            if write_header is True:
                data.to_csv('./data/aqi_data.csv', index=False, mode='a', header=True)
                write_header = False
            else:
                data.to_csv('./data/aqi_data.csv', index=False, mode='a', header=False)
