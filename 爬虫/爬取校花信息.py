...
class XHSpider():
    ...
    def loadpage(self):
        ...
        '''
        注意：如果发现用xpath插件在浏览器上取到标签，但是在代码里面娶不到
        ...
        for div in div_list:
            ...
            # 开始请求详情页，把标题传过去，后面有用
            self.load_desc_page(desc_url, img_title)

    # 详情页
    def load_desc_page(self, desc_url, img_title):
        # 发起请求
        response = requests.get(url=desc_url, headers=self.headers)
        ...
        # 开始请求校花空间
        self.load_zone(more, img_title)

    # 校花空间提取
    def load_zone(self, more, img_title):
        # 发起请求
        response = requests.get(url=more, headers=self.headers)
        ...
        # 取出图片的地址列表
        big_img_list = html.xpath('//div[@class="inner"]//a/@href')

        # 做图片地址的容错处理
        for big_img in big_img_list:
    ...
    def download(self, big_img, img_title):
        print('正在下载', big_img)
        # 发起请求
        response = requests.get(url=big_img, headers=self.headers)
...
if __name__ == '__main__':
    xhs = XHSpider()
    # 这里正确的额逻辑应该自动提取下一页，然后自动加载，不过数据量不大，可以简单通过循环提取
    # 这里只爬取两个
    for i in range(0, 2):
        print('爬取第%d页' % i)
        xhs.pn = i  # 把每页赋值给pn
